...
from sqlalchemy import (
    Boolean, Enum, ForeignKey, Integer, JSON, String, Text,
...
class Workspace(Base):
    ...
    plan: Mapped[Plan] = mapped_column(Enum(Plan, name="plan"), nullable=False, server_default=text("'free'"))
    stripe_customer_id: Mapped[Optional[str]] = mapped_column(String(64), unique=True, nullable=True)
    monthly_render_quota: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("10"))
    created_at: Mapped[datetime] = mapped_column(nullable=False, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(nullable=False, server_default=func.now(), onupdate=func.now())
    ...
    renders: Mapped[list[Render]] = relationship("Render", back_populates="workspace", cascade="all, delete-orphan")
    quota_usage: Mapped[list[QuotaUsage]] = relationship("QuotaUsage", back_populates="workspace", cascade="all, delete-orphan")
    subscription: Mapped[Optional[Subscription]] = relationship("Subscription", back_populates="workspace", uselist=False, cascade="all, delete-orphan")
    webhook_endpoints: Mapped[list[WebhookEndpoint]] = relationship("WebhookEndpoint", back_populates="workspace", cascade="all, delete-orphan")
...
class ApiKey(Base):
    ...
    status: Mapped[ApiKeyStatus] = mapped_column(Enum(ApiKeyStatus, name="api_key_status"), nullable=False, server_default=text("'active'"))
    last_used_at: Mapped[Optional[datetime]] = mapped_column(nullable=True)
    created_at: Mapped[datetime] = mapped_column(nullable=False, server_default=func.now())
    revoked_at: Mapped[Optional[datetime]] = mapped_column(nullable=True)

    __table_args__ = (
        Index("api_keys_workspace_idx", "workspace_id"),
    )
...
class Render(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    workspace_id: Mapped[int] = mapped_column(Integer, ForeignKey("workspaces.id", ondelete="cascade"), nullable=False)
    api_key_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("api_keys.id", ondelete="set null"), nullable=True)
    ark_task_id: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    prompt: Mapped[str] = mapped_column(Text, nullable=False)
    model: Mapped[str] = mapped_column(String(64), nullable=False, server_default=text("'doubao-seedance-2-0-260128'"))
    ...
    status: Mapped[RenderStatus] = mapped_column(Enum(RenderStatus, name="render_status"), nullable=False, server_default=text("'queued'"))
    video_url: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    error: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    cost_cents: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("0"))
    extra_metadata: Mapped[Optional[dict[str, Any]]] = mapped_column(JSON, name="extra_metadata", nullable=True)
    created_at: Mapped[datetime] = mapped_column(nullable=False, server_default=func.now())
    ...
    api_key: Mapped[Optional[ApiKey]] = relationship("ApiKey", back_populates="renders")
...
class QuotaUsage(Base):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "period_start", name="quota_usage_unique"),
    )
...
class Subscription(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    workspace_id: Mapped[int] = mapped_column(Integer, ForeignKey("workspaces.id", ondelete="cascade"), unique=True, nullable=False)
...
class WebhookEndpoint(Base):
    ...
    events: Mapped[list[str]] = mapped_column(JSON, nullable=False, server_default=text("'[\"render.succeeded\",\"render.failed\"]'::json"))
...
class AuditLog(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    workspace_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("workspaces.id", ondelete="set null"), nullable=True)
    api_key_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("api_keys.id", ondelete="set null"), nullable=True)
